...
def reject_low_region(box, frame_h, thr=0.75):
    """
    Reject people whose bounding box is mostly in the lower part of the frame.
    thr = 0.75 means bottom 25% is ignored.
    """
    x1, y1, x2, y2 = box
    center_y = (y1 + y2) / 2
    return center_y < frame_h * thr
...
def run_sequence(predictor, video_path, frame_names, writer, save_vis=True, quiet=False):
    ...
    det_results = det_model(first_frame, verbose=False)
    # load first frame to get height
    first_frame_img = cv2.imread(first_frame)
    frame_h = first_frame_img.shape[0]
    ...
    bboxes = [box.tolist()
              ...
              if int(cls) == 0
              and float(conf) >= 0.4
              and reject_low_region(box, frame_h, thr=0.75)]  # ✅ REMOVE AUDIENCE
    ...
    with torch.no_grad():
        for rel_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state):
            ...
            det_results = det_model(frame_path, verbose=False)
            ...
            cur_boxes = [
                ...
                if int(cls) == 0
                   and float(conf) >= 0.4
                   and reject_low_region(box, frame.shape[0], thr=0.75)  # ✅ REMOVE AUDIENCE
            ]
...
def main():
    ...
    for seq in sorted(os.listdir(split_dir)):
        img_dir = os.path.join(split_dir, seq, "img1_part")
