...
def load_detections(det_dir, frame_id):
    """Load YOLOX detection for a given frame, returns [[x1,y1,x2,y2], ...]"""
    txt_path = os.path.join(det_dir, f"{frame_id:06d}.txt")
...
def run_sequence(predictor, video_path, frame_names, det_dir, offset, writer, save_vis=True, quiet=False):
    """Run SAM2+YOLO tracking with tqdm progress and frame saving only."""
    inference_state = predictor.init_state(video_path=video_path)
    predictor.reset_state(inference_state)

    # --- Initial detection ---
    first_id = int(os.path.splitext(frame_names[0])[0])
    bboxes = load_detections(det_dir, first_id)
    ...
    with torch.no_grad():
        for rel_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state):
            ...
            frame_id = int(os.path.splitext(frame_names[rel_idx])[0])
            frame_path = os.path.join(video_path, frame_names[rel_idx])
            frame = cv2.imread(frame_path)

            # Load detections from precomputed folder
            cur_boxes = load_detections(det_dir, frame_id)
            ...
            if save_vis:
                save_path = os.path.join(vis_dir, f"{abs_idx:06d}.jpg")
...
def main():
    ...
    for seq in sorted(os.listdir(split_dir)):
        ...
        det_dir = os.path.join(detection_root, seq)
        if not os.path.isdir(img_dir) or not os.path.isdir(det_dir):
            continue
        #test_list = sorted(os.listdir(split_dir))[4]
        ...
        with open(out_file, "w") as f:
            ...
            if num_frames <= MAX_DIRECT:
                ...
                run_sequence(predictor, img_dir, frame_names, det_dir, offset=0, writer=f, save_vis=False, quiet=False)
            ...
            # Sliding mode
                ...
                while frame_idx < num_frames:
                    ...
                    run_sequence(predictor, TMP_DIR, slice_frames, det_dir, offset=frame_idx, writer=f, save_vis=False, quiet=False)
...
if __name__ == "__main__":
    ...
    detection_root = "detections_yolox_x/test_fixed"
