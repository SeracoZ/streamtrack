...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_object(
        ...
        """
        Dynamically add a new object into the current inference_state mid-tracking.
        Mirrors remove_object(), but for addition.
        """
        # 1️⃣ Create object slot if not exists
        if obj_id in inference_state["obj_id_to_idx"]:
            if verbose:
                print(f"[add_object] Object {obj_id} already exists, skipping.")
            return inference_state
        obj_idx = len(inference_state["obj_id_to_idx"])
        inference_state["obj_id_to_idx"][obj_id] = obj_idx
        inference_state["obj_idx_to_id"][obj_idx] = obj_id
        ...
        # 2️⃣ Initialize per-object containers
        inference_state["point_inputs_per_obj"][obj_idx] = {}
        inference_state["mask_inputs_per_obj"][obj_idx] = {}
        inference_state["output_dict_per_obj"][obj_idx] = {
            "cond_frame_outputs": {},
        ...
        # 3️⃣ Add prompt (box or points) to initialize the mask
        ...
        # 4️⃣ Run memory encoder manually (like propagate_in_video_preflight)
        out = inference_state["temp_output_dict_per_obj"][obj_idx]["cond_frame_outputs"].get(frame_idx)
        if out is None:
            raise RuntimeError(f"[add_object] No temporary mask found for obj {obj_id} at frame {frame_idx}.")
        ...
        # 5️⃣ Promote temporary output to the active memory
        ...
        # 6️⃣ Finalize mapping and sync IDs
        inference_state["obj_ids"] = list(inference_state["obj_id_to_idx"])

        if verbose:
            print(f"✅ Added object {obj_id} (idx {obj_idx}) at frame {frame_idx}")
        return inference_state
